...
async function main() {
  ...
  const comment1 = await prisma.comments.create({
    data: {
      authorId: bob.id,
      postId: aliceBlog.id,
      content: "Heloo comment from bob",
    },
  });
  ...
  const comment2 = await prisma.comments.create({
    data: {
      authorId: alice.id,
      postId: bobBlog.id,
      content: "Heloo comment from alice",
    },
  });
}
